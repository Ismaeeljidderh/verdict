...
def _analyze_url(raw):
    ...
    # If this IS an official verified domain, skip all typosquatting checks —
    # it can never be "similar to itself" or a false-positive match.
    is_official_domain = bare in {re.sub(r'^www\.', '', d) for d in BRAND_DOMAINS}
    ...
    if not is_official_domain:
        for bd in BRAND_DOMAINS:
            b = re.sub(r'^www\.', '', bd)
            if bare == b:
                continue  # exact match to a verified domain — never flag
            sim = SequenceMatcher(None, bare, b).ratio()
            # Require high similarity AND similar length to avoid short-name false positives
            if 0.85 < sim < 1.0 and abs(len(bare)-len(b)) <= 2 and not details["brand_match"]:
                flags.append(f"Very similar to {bd} — possible typosquatting"); score+=20; details["brand_match"]=bd
            key = b.split('.')[0]
            if len(key) >= 4 and key in bare and bare != b and not details["brand_match"]:
